...
ZMBKPOSE_SCRIPT=/home/smartitbg/zmbkpose/zmbkpose.sh
...
if [ $RUNNING = "err" ]; then
  echo "ERROR: Last run triggered an error. Stopping."
  # send mail
  sendmail admin@easycredit.bg < /etc/scripts/zmbkpose/email.txt
  exit 1
...
ldapsearch -x -D "cn=config" -w $LDAPPASS -h $LDAPHOST '(&(zimbraAccountStatus=Active))' 2> /dev/null | grep "mail:" | awk '{print $2}' > $ACCOUNTS_FILE
...
if [ -f $ACCOUNTS_FILE ]; then
  ...
  if [ $? -ne 0 ]; then
    echo "ERROR: $ACCOUNTS_FILE does not contains valid email address."
...
while read account; do
  ...
  if [ $COUNTER -eq 5 -o $OVERALL_COUNTER -eq $NUM_OF_ACCOUNTS ]; then
    ...
    $ZMBKPOSE_SCRIPT $1 -t -a $CAT_ACCOUNTS &>> $LOG_FILE &
    # timing
    sleep 4
